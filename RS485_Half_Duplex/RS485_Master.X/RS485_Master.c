...
char buffer_rx[10];                                 // Almacena los datos recibidos del esclavo
char buffer_tx[10];                                 // Almacena los datos para enviar al esclavo
short bit_flag = 1;
...
void main()
{
    ...
    while(1)
    {
        while(bit_flag == 1)
        {
            RS_MODE_PIN = 1;                        // Modo transmisor  (1)
            char v_dato = PORTB & 0x0F;
            sprintf(buffer_tx, "%u\r\n", v_dato);
            Uart_Send_String(buffer_tx);            // Envia los datos al esclavo
            __delay_ms(10);
            bit_flag = 0;
        }
        
        RS_MODE_DIR = 1;                            // Modo receptor      (0)
        Uart_Read_String(buffer_rx, 10);            // Lee el dato del esclavo
        if(strlen(buffer_rx) > 0)                   // Verifica si ha recibido los datos
        {
